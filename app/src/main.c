...
void touch_init(void)
{
    ...
    uint8_t config_data[] = {/* Configuration data */};
    i2c_write(i2c_dev, config_data, sizeof(config_data), FT5406_I2C_ADDR);
...
void touch_read(lv_indev_data_t *data)
{
    ...
    i2c_read(i2c_dev, touch_data, sizeof(touch_data), FT5406_I2C_ADDR);

    /* Parse touch data */
    uint16_t x = (touch_data[1] << 8) | touch_data[0];
    uint16_t y = (touch_data[3] << 8) | touch_data[2];
    bool pressed = (touch_data[5] & 0x01);
...
int main(void)
{
    ...
    lv_obj_t * t2 = lv_tabview_add_tab(tv, "Board Stats");
...
static void profile_create(lv_obj_t * parent)
{
    ...
    lv_label_set_text(name, "Kohler Energy GM121395 Testing");
    lv_obj_add_style(name, &style_title, 0);

    lv_obj_t * dsc = lv_label_create(panel1);
    lv_obj_add_style(dsc, &style_text_muted, 0);
    lv_label_set_text(dsc, "Main UI for testing. Managed by blockhouse engineering.");
    ...
    lv_label_set_text(email_label, "richard.castro@kohler.com");
    ...
    lv_label_set_text(call_label, "920-226-4626");
    ...
    lv_label_set_text(label, "Sign Out");
    lv_obj_center(label);
